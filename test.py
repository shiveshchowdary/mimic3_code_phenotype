...
class MimicDataSetPhenotyping(Dataset):
    ...
    def extract(self, values, id_name_dict):
        ...
        if self.uniform:
            variable, value, time = self.limit_feature_appearances(variable, value, time, 60)
...
model.load_state_dict(torch.load('models/fine_tuned_versionlatest_256_model.pth'))
...
for k in range(K):
    ...
    macro, micro, test_loss = calculate_roc_auc(model, test_dataloader)
    res_dict["macro"].append(macro)
    res_dict["micro"].append(micro)

with open("base_line_latest_finetuned_256.json","w") as f:
...
for k in range(K):
    ...
    res_dict["micro"].append(micro)
import json
with open("base_line_uniform_finetuned_256.json","w") as f:
    json.dump(res_dict, f)
...
for k in range(K):
    ...
    res_dict["micro"].append(micro)
import json
with open("base_line_random_finetuned_256.json","w") as f:
    json.dump(res_dict, f)